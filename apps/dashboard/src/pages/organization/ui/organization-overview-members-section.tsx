...
function MembersSectionCardHeader() {
	return (
		<CardHeader>
			...
			<CardDescription>
				{translate("dashboard.routes.organization.recent-members.description")}
			</CardDescription>
		</CardHeader>
	);
}
						...
						<ItemGroup>
							{recentMembers.map((member, index) => (
								<Fragment key={member.id}>
									...
									{index !== recentMembers.length - 1 && <ItemSeparator />}
								</Fragment>
							))}
						</ItemGroup>
...
export function OrganizationOverviewMembersSection() {
	return (
		<ErrorBoundary FallbackComponent={OrganizationMembersSectionErrorFallback}>
