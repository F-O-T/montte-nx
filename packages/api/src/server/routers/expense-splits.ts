...
export const expenseSplitsRouter = router({
	...
	delete: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (!existingSplit || existingSplit.organizationId !== organizationId) {
	...
	getByTeam: protectedProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			try {
				return await findExpenseSplitsByTeamId(resolvedCtx.db, input.teamId);
			} catch (error) {
				propagateError(error);
	...
	getMyBalance: protectedProcedure.query(async ({ ctx }) => {
		...
		try {
			return await getMemberBalance(resolvedCtx.db, member.id, organizationId);
		} catch (error) {
			propagateError(error);
