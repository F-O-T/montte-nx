   ...
   try {
      const result = await dbClient.transaction(async (tx) => {
         ...
         if (participants.length > 0) {
            ...
            await tx
               .insert(expenseSplitParticipant)
               .values(participantsWithSplitId);
         }
...
export async function findExpenseSplitsByMemberId(
   ...
   try {
      const participantRecords =
         await dbClient.query.expenseSplitParticipant.findMany({
...
export async function getMemberBalance(
   ...
   try {
      ...
      for (const participation of participations) {
         ...
         const remaining =
            Number(participation.allocatedAmount) -
            Number(participation.settledAmount);
         if (remaining > 0) {
            totalOwed += remaining;
         }
      }
      ...
      for (const split of createdSplits) {
         for (const participant of split.participants) {
            ...
            const remaining =
               Number(participant.allocatedAmount) -
               Number(participant.settledAmount);
            if (remaining > 0) {
