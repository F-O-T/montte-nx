import { createMoney } from "../core/internal";
...
export function allocate(money: Money, ratios: number[]): Money[] {
	...
	for (let i = 0; i < ratios.length; i++) {
		...
		// ideal = (amount * ratio / totalRatio) * precisionFactor
		const scaledRatio = BigInt(Math.round(ratio * 1e10));
		const scaledTotalRatio = BigInt(Math.round(totalRatio * 1e10));
	...
	while (remaining > 0n) {
		...
		const entry = sortedByRemainder[i % sortedByRemainder.length]!;
		entry.amount += 1n;
		remaining -= 1n;
		i++;
	}
	...
	while (remaining < 0n) {
		...
		const entry = sortedByRemainder[sortedByRemainder.length - 1 - (i % sortedByRemainder.length)]!;
		if (entry.amount > 0n) {
			entry.amount -= 1n;
			remaining += 1n;
		}
